...
const ball = pads.find(p => p.net === NET && p.padNumber === 'AC5');
const cap  = pads.find(p => p.net === NET && p.padNumber !== 'AC5');
const out = { ball: [+ball.x.toFixed(2), +ball.y.toFixed(2)], cap: [+cap.x.toFixed(2), +cap.y.toFixed(2)], capPad: JSON.stringify(cap.pad) };
// ---------- 1) AC5 端：dogbone 短桩 + 孔 ----------
function obsNear(cx, cy, R) { const O=[];
  for (const l of lines) if (l.net !== NET) { if (segPt(l.startX,l.startY,l.endX,l.endY,cx,cy) < R)
  ...
  return O; }
const OB = obsNear(ball.x, ball.y, 70);
out.obsBall = OB.length;
function minSlack(O, x, y, r, skipBall) { let w = 1e9, who = '';
  for (const o of O) { const s = o.sd(x,y) - r - CL; if (s < w) { w = s; who = o.tag; } } return { w, who }; }
...
for (const r of [7, 6.5, 6, 8]) {
  for (let a = 0; a < 360; a += 5) for (let dist = 7; dist <= 22; dist += 1) {
    const rad = a * Math.PI / 180, vx = +(ball.x + dist*Math.cos(rad)).toFixed(2), vy = +(ball.y + dist*Math.sin(rad)).toFixed(2);
    if (padSD(ball, vx, vy) < r + 0.6) continue;             // 必须完全在球外（dogbone）
    const s = minSlack(OB, vx, vy, r); if (s.w < 0) continue;
    if (!stubOK(OB, ball.x, ball.y, vx, vy)) continue;
    const sc = s.w - 0.15 * dist;
    if (!best || sc > best.sc) best = { r, vx, vy, dist, slack: +s.w.toFixed(2), who: s.who, sc };
  }
  if (best) break;
}
out.dogbone = best;
...
for (const r of [8, 7]) {
  for (let i = -20; i <= 20; i++) for (let j = -20; j <= 20; j++) {
    ...
    if (padSD(cap, x, y) > r - 1) continue;                  // 与电容焊盘重叠
    const s = minSlack(OC, x, y, r);
    if (s.w < 0) continue;
...
// ---------- 3) 层16 A* ----------
const A = [best.vx, best.vy], Bv = [bestC.x, bestC.y];
const o16 = [];
for (const l of lines) if (l.net !== NET && l.layer === 16) { const d = segPt(l.startX,l.startY,l.endX,l.endY,(A[0]+Bv[0])/2,(A[1]+Bv[1])/2);
  if (d < 250) o16.push({ tag:'L16 '+l.net, sd:(x,y)=>segPt(l.startX,l.startY,l.endX,l.endY,x,y)-num(l.lineWidth)/2 }); }
for (const v of vias) if (v.net !== NET) { if (Math.hypot(v.x-(A[0]+Bv[0])/2, v.y-(A[1]+Bv[1])/2) < 250) o16.push({ tag:'via', sd:(x,y)=>Math.hypot(v.x-x,v.y-y)-num(v.diameter)/2 }); }
for (const p of pads) if (p.net !== NET && p.layer === 12) o16.push({ tag:'thpad', sd:(x,y)=>padSD(p,x,y) });
...
const si2 = toI(A[0],A[1]), ti2 = toI(Bv[0],Bv[1]);
fr[si2[1]*nx+si2[0]] = 1; fr[ti2[1]*nx+ti2[0]] = 1;
...
const kS2 = si2[1]*nx+si2[0], kT2 = ti2[1]*nx+ti2[0];
gv[kS2]=0; fv[kS2]=Math.hypot(ti2[0]-si2[0],ti2[1]-si2[1]); hpush(kS2);
...
while (H.length) { const k=hpop(); pops++;
  ...
  for (let d=0;d<8;d++){ const ni=ci+DX[d], nj=cj+DY[d];
    ...
    const ng=gc+DW[d]; if(ng<gv[kk]-1e-6){ gv[kk]=ng; pv[kk]=k; fv[kk]=ng+Math.hypot(ti2[0]-ni,ti2[1]-nj); hpush(kk); } }
...
let path=[], cur=kT2; while(cur>=0){ const i=cur%nx, j=(cur-i)/nx; path.push([+(BX[0]+i).toFixed(1), +(BX[1]+j).toFixed(1)]); cur=pv[cur]; }
path.reverse(); path[0]=A; path[path.length-1]=Bv;
function clr(x1,y1,x2,y2){ const L=Math.hypot(x2-x1,y2-y1), n=Math.ceil(L/1.0);
